...
export class UserService {
  ...
  async update(id: string, update: UpdateUserDTO): Promise<UserDocument | null> {
    ...
    const emailExists = user.emailList.some(
      (entry) => entry.email === update.email
    );
  
    if (!emailExists) {
      user.emailList.push({
        email: update.email, count: update.count, name: update.name, position: update.position, signature: update.signature,
